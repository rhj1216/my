...
import com.example.myapp.myapplication.Utility;
import com.example.myapp.myapplication.weather.WeatherAboutLife;
import com.example.myapp.myapplication.weather.WeatherForecast;
...
//                Log.i(TAG, "onLocationChanged: "+aMapLocation.getAddress());
                if(flag==false){
                    Log.i(TAG, "onLocationChanged: +++++++成功");
                    getAir();
                    getWeatherNow();
    ...
    public void getPermission(){
        ...
        Log.i(TAG, "getPermission: "+Arrays.toString(permissionArray));
    ...
    public void getWeatherNow()  {
        ...
        /*    //当前天气状况请求(sdk)*/
        HeWeather.getWeatherNow(this, location, new HeWeather.OnResultWeatherNowBeanListener() {
            @Override
...
//                String[] strArr=new String[]{"1","2","3","4","5","6",};
                String location=list.get(0).getBasic().getLocation();//城市
                String weather = list.get(0).getNow().getCond_txt();//天气
                ...
                //把数据添加进datList里
                dataList.add(0,"城市: "+location);
                dataList.add(1,"天气: "+weather);
                dataList.add(2,"体感温度: "+fi);
                dataList.add(3,"温度: "+tmp);
                dataList.add(4,"湿度: "+hum);
                dataList.add(5,"降水量: "+pcpn);
...
//            Response response=call.execute();//同步请求
    ...
    public void getLife(){
        Log.i(TAG, "getLife: --------------");
        HeWeather.getWeatherLifeStyle(this, location,new HeWeather.OnResultWeatherLifeStyleBeanListener() {
            @Override
            public void onError(Throwable throwable) {
...
//
    ...
    public void getWeatherForecast(){
        ...
        HeWeather.getWeatherForecast(this,location ,new HeWeather.OnResultWeatherForecastBeanListener() {
            @Override
            public void onError(Throwable throwable) {
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.getLife:
                Log.i(TAG, "onClick: ---------------------生活指数");
                getLife();
                break;
            case R.id.getWeatherForecast:
                Log.i(TAG, "onClick: ---------------------未来三天天气");
                getWeatherForecast();
                break;
        }
    }
